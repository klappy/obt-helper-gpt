...
import { generateSummary, summarizeAndStore, fetchUserSummaries } from './openai.js';
...
describe('Issue 1.1.2: LLM Summary Generation', () => {
  ...
  describe('generateSummary', () => {
    it('should call OpenAI API with correct parameters', async () => {
      ...
      const chatHistory = 'user: How do I make better pasta?\nassistant: Try using salted water and...';
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(generateSummary('test chat')).rejects.toThrow('OpenAI API error: 401 Unauthorized');
    });
  });
    ...
    it('should retrieve and sort summaries by timestamp', async () => {
      ...
      const mockStore = {
        list: vi.fn().mockResolvedValue({
          blobs: [
            { key: 'summary-session1' },
            { key: 'summary-session2' }
          ]
        }),
        get: vi.fn()
